...
export async function saveBoardingPassCard(submissionId: string, blob: Blob): Promise<void> {
  const formData = new FormData();
  formData.append('image', blob, `${submissionId}.png`);
  formData.append('submissionId', submissionId);

  const response = await fetch('/api/save-boarding-pass', {
    method: 'POST',
    body: formData,
  });

  if (!response.ok) {
    throw new Error('Failed to save boarding pass image');
  }
}
